...
class GenerateExpensesReportPdfUseCase : IGenerateExpensesReportPdfUseCase
{
    ...
    {
        var expenses = await _reposotory.FilterByMonth(month);
        if (expenses.Count == 0)
        {
            return [];
        }
        ...
        foreach (var expense in expenses)
        {
            ...
            {
                var decriptionRow = table.AddRow();
                decriptionRow.Height = HEIGHT_ROW_EXPENSE_TABLE;

                decriptionRow.Cells[0].AddParagraph(expense.Description);
                decriptionRow.Cells[0].Format.Font = new Font
                {
                    Name = FontHelper.WORKSANS_REGULAR,
                    Size = 10,
                    Color = ColorsHelper.BLACK
                };
                decriptionRow.Cells[0].Shading.Color = ColorsHelper.GREEN_LIGHT;
                decriptionRow.Cells[0].VerticalAlignment = VerticalAlignment.Center;
                decriptionRow.Cells[0].MergeRight = 2;
                decriptionRow.Cells[0].Format.LeftIndent = 20;
    ...
    private Document CreateDocument(DateOnly month)
    {
        ...
        document.Info.Author = "Raphael Aciardi";
    ...
    private void CreateHeaderWithProfilePhotoAndName(Section page)
    {
        ...
        var text = row.Cells[1].AddParagraph("Hey, Raphael Aciardi");
        text.Format.Font = new Font { Name = FontHelper.RALEWAY_BLACK, Size = 16 };
